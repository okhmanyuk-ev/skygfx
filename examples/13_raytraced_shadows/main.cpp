...
int main()
{
	...
	auto [window, native_window, width, height] = utils::SpawnWindow(800, 600, "Raytraced shadows");
	...
	auto shader = skygfx::RaytracingShader(raygen_shader_code, { miss_shader_code, shadow_miss_shader_code }, closesthit_shader_code);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		auto static_blas = skygfx::BLAS(vertices, 0, indices, 0, static_model);

		auto tlas = skygfx::TLAS({
			blas,
			static_blas,
		});
		...
		skygfx::SetUniformBuffer(6, light);
